 ...
 * OopsComments.vue
 * - props
 *   - oopsId:      대상 OOPS 게시글 ID (필수)
 *   - initialComments: 상세 API에서 받은 초기 comments 배열(옵션) — API 실패/빈배열일 때 fallback
 * - emits
 *   - update:list  (list: Comment[])  → 부모(oops.comments) 동기화
 *   - update:count (n: number)        → 부모(oops.commentsTotal) 동기화
 *
 * 사용 API
...
async function send() {
  ...
  try {
console.log("oops기록 ID:", props.oopsId);
